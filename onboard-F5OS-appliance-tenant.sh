#!/bin/bash
# onboard-F5OS-appliance-tenant.sh will deploy a tenant on an rSeries (possibly not a VELOS since blade parameter is a pre-req) 
# and then perform the necessary onboarding directly. 
#
# The steps the scripts takes are as follows
# . Communicate with a rSeries F5OS appliance
# . Create a Tenant 
...
while getopts "h" opt; do
    case $opt in
        h)
            echo ""
            echo "onboard-F5OS-appliance-tenant.sh <newTenantIP> <lastOctectProductionInterfaces> <TargetF5OSname> <NewTenantFQDN> <DFG-Mgt-Net>"
            echo "" 
            exit 1
            ;;
   esac
...
echo ""
curl -sku admin:admin https://$1/mgmt/shared/authz/users/admin -X PATCH -H "Content-type: application/json" -d '{"oldPassword":"admin", "password":'\"$pw\"'}'|jq
sleep 1
echo "Get Token"
echo "_________"
...
curl -s -ku $CREDS https://$IP/mgmt/shared/file-transfer/uploads/$FN -H 'Content-Type: application/octet-stream' -H "Content-Range: 0-$((LEN - 1))/$LEN" -H "Content-Length: $LEN" -H 'Connection: keep-alive' --data-binary @$FN
DATA="{\"operation\":\"INSTALL\",\"packageFilePath\":\"/var/config/rest/downloads/$FN\"}"
curl -s -ku $CREDS "https://$IP/mgmt/shared/iapp/package-management-tasks" -H "Origin: https://$IP" -H 'Content-Type: application/json;charset=UTF-8' --data $DATA
cd $HOME/Documents/Scripts/API-F5OS/F5OS-DO-tenant
echo ""
echo "Pushed DO rpm to tenant"
echo ""
sleep $wait
echo ""
...
-k "https://$1/mgmt/shared/declarative-onboarding" 
